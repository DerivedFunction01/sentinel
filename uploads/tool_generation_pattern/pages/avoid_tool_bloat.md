...
### Advanced Pattern: State-Based Filtering with Revision IDs

For complex domains (finance, retail, inventory) where tools operate on **filtered states that evolve**, use a **version-control-like pattern** with `revision_id` and `filter_delta` instead of creating tools with deep parameter nesting or repeating filtering logic.

**The Pattern:**

1. **One Filtering/Query Tool** that accepts:
   - `revision_id` (nullable): Reference to a previous query result (enables branching/undo)
   - `filter_delta`: Additive and subtractive filter operations (`add_filters`, `remove_filters`)
   - Returns a new `revision_id` representing the filtered state

2. **Specialized Downstream Tools** that accept:
   - `revision_id` (required): A reference to filtered data from step 1
...
Tool 1: apply_filters
  revision_id: (optional reference to previous state)
  filter_delta: { add: [...], remove: [...] }
  → returns revision_id

Tool 2: kitchen_display
  revision_id: (required, from Tool 1)
  format_hints: {...}
...
Or, a single tool that that accepts a type of view or operation, and the filter `revision_id`
...
- **Undo/Branch**: LLM can reference an old `revision_id` + apply new filters to explore alternate paths without recomputing
- **State Sharing**: Same filtered result (`revision_id`) passed to multiple tools without duplication
...
```
Query 1: apply_filters(revision_id=null, add_filters=[{category="electronics"}])
  → revision_id: "v1_electronics"

Query 2: apply_filters(revision_id="v1_electronics", add_filters=[{price_range="0-500"}])
  → revision_id: "v2_electronics_budget"

Query 3: kitchen_display(revision_id="v2_electronics_budget")
  → Displays filtered results in KDS format

Query 4: tax_report(revision_id="v2_electronics_budget")
  → Aggregates tax liability for the same filtered set

Query 5: apply_filters(revision_id="v1_electronics", add_filters=[{price_range="500-2000"}])
  → revision_id: "v3_electronics_premium"  (branched from v1, not v2)

Query 6: inventory_rebalance(revision_id="v3_electronics_premium", mutation_batch=[...])
...
- LLM must understand that `revision_id` is a reference, not a value to inspect
- Mock responses for filtering tool return only the `revision_id` and metadata, not the filtered data itself
- Downstream tools accept `revision_id` and operate on backend's materialized state

### Safe Filtering: Enumerated Operators (Safe SQL Pattern)

The filtering pattern is inherently safer than free-form query strings because **filters are restricted to enumerated operators**, not arbitrary code or SQL.

**Core Operators:**

- **Equality**: `equals`, `not_equals`
- **Set Operations**: `in_set`, `not_in_set`
- **Comparison**: `gt`, `geq`, `lt`, `leq`
- **String Matching**: `like`, `not_like` (regex or pattern-based, backend-defined)
...
vs. unsafe alternative:

```
"filter": "SELECT * WHERE price >= 100 OR 1=1"  // ✗ SQL injection risk
```

**Why This Matters:**

1. **Injection Proof**: LLM cannot craft SQL or code; only enum values and property names (also validated)
2. **Deterministic**: Backend always knows what operation is being requested
3. **Auditable**: Filter history is a series of safe, atomic operations
4. **Composable**: Filters combine predictably; no surprise interactions

**Filter Safety Rules:**

- `property` is restricted to an enum (e.g., `["name", "sku", "price", "quantity", "category", "brand"]`)
- `operator` is restricted to an enum (e.g., `["eq", "neq", "in_set", "gt", "like"]`)
- `value` is typed (string, number, or array of strings/numbers); backend validates against property type
- No string interpolation, template variables, or code execution
- Filters are combined using simple AND/OR logic (defined by backend, not LLM)
...
**Referencing the new `revision_id` to add another filter:**
...
Backend compiles to: `(category IN ['electronics', 'appliances']) AND (price < 500) AND (brand NOT LIKE '%BrandA%')`

**Why Enums Over Strings:**

- `operator: "gt"` (enum) vs. `operator: ">"` (string interpreted by backend)
- Enums are validated by the tool definition; strings require runtime parsing
- Prevents typos, accidental operators, and interpretation drift
- Forces LLM to pick from known, safe operations

**Design Principle: Constrain at the Tool Schema Level**

Instead of teaching the LLM "don't do SQL injection", the tool schema **makes injection impossible**:
          ...
          "items": {
            ...
            "properties": {
              "property": {
                ...
                // ✓ Only these properties allowed; no free-form strings
              },
              "operator": {
                "type": "string",
                "enum": [
...
The schema itself is the enforcement. The LLM cannot violate it.

**Benefits Over Alternatives:**

| Approach                                  | Safety              | Composability        | Auditability       |
| ----------------------------------------- | ------------------- | -------------------- | ------------------ |
| Free-form query string                    | ✗ (injection risk)  | ✗ (parsing complex)  | ✗ (opaque)         |
| String-based operators ("sql", "mongodb") | ⚠ (risky)           | ⚠ (dialect-specific) | ⚠ (parsing needed) |
| **Enumerated operators**                  | ✓ (schema-enforced) | ✓ (composable atoms) | ✓ (clear trail)    |
